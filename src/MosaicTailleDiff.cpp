...
int findBestImagette(OCTET *bloc,OCTET *currentImagette,int nH_bloc,int nW_bloc,int precision){
	...
	if(indiceBestImagette==-1){
		return findBestImagette(bloc,currentImagette,nH_bloc,nW_bloc,precision+10);
	}
	printf("precision=%d\n",precision);
...
void imagetteMosaic_moy(OCTET* imageIn, OCTET* imageOut, int nH, int nW)
{
	...
	while(i!=-1 && j!=-1){
		...
		if(ecartType(bloc,nW_bloc,nH_bloc)>5 && n<4){
			n+=1;
			continue;
		}

		int indiceBestImagette=findBestImagette(bloc,currentImagette,nH_bloc,nW_bloc,10);
